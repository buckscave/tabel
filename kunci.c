...
/* ============================================================
 * Helper: Render Input Line (newline -> \n)
 * ============================================================ */
void render_input_line(const char *src, int len, int pos_x_input, int pos_y) {
    char tampil[MAKS_TEKS * 2];
    int j = 0;
    int i;
    /* Dapatkan batas ujung kanan jendela agar tidak menyapu jendela sebelahnya */
    ...
    /* Bersihkan area input hanya sebatas lebar jendela aktif */
    ...
    atur_posisi_kursor(pos_x_input, pos_y);
    if (j > 0) {
        /* Potong karakter visual jika melebihi batas window */
        int ruang = max_x - pos_x_input + 1;
        if (j > ruang && ruang > 0) j = ruang;
        write(STDOUT_FILENO, tampil, (size_t)j);
    }
}
...
int baca_input_edit(char *buf, int *len, int *cursor,
    ...
    while (1) {
        ...
        /* Redraw Input Line */
        max_x = jendela_aktif ? 
            (jendela_aktif->rect.x + jendela_aktif->rect.w) : 
            ambil_lebar_terminal();
        atur_posisi_kursor(pos_x_input, pos_y);
        for (i = pos_x_input; i <= max_x; i++) {
            write(STDOUT_FILENO, " ", 1);
        }
        atur_posisi_kursor(pos_x_input, pos_y);
        render_input_line(buf, *len, pos_x_input, pos_y);
        ...
        if (*cursor >= 0) {
            ...
            atur_posisi_kursor(pos_x_input + lebar_v, pos_y);
        } else {
            atur_posisi_kursor(pos_x_input, pos_y);
        }
        flush_stdout();
...
void mode_edit_berantai(struct buffer_tabel *buf, int x_mulai,
    ...
    while (sedang_edit) {
        ...
        kolom_layar = ambil_lebar_terminal();

        atur_posisi_kursor(win_x + 1, win_y + 1);
        for (i = 0; i < win_w; i++) {
            write(STDOUT_FILENO, " ", 1);
        }
        atur_posisi_kursor(win_x + 2, win_y + 1);
        write(STDOUT_FILENO, label, strlen(label));
        render_input_line(buffer_edit, panjang, pos_x_input, win_y + 1);
        ...
        atur_posisi_kursor(pos_x_input + visual_offset, win_y + 1);
