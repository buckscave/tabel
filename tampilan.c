                                   ...
                                   struct rect area, int is_active)
{
    ...
    if (buf && buf->lembar && buf->jumlah_lembar > 0) {
        ...
        /* Hitung lebar total untuk sheet tabs yang visible */
        lebar_total = 0;
        for (i = vis_awal; i <= vis_akhir; i++) {
            lebar_total += (int)strlen(buf->lembar[i]->nama) + 2; /* spasi kiri-kanan */
        }

        /* Tambah ruang untuk indikator jika lebih dari 3 lembar */
        if (ada_lebih_kiri) {
            lebar_total += 1; /* "<" */
        }
        if (ada_lebih_kanan) {
        ...
        /* Posisi mulai sheet tabs - pastikan tab terakhir mepet kanan */
        awal_lembar = kolom - lebar_total + 1;
        ...
        for (i = vis_awal; i <= vis_akhir && pos_tab <= kolom; i++) {
            ...
            /* Tulis nama tab */
            if (i == vis_akhir && !ada_lebih_kanan) {
                /* Tab terakhir tanpa indikator: isi sampai ujung kanan */
                int j;
                for (j = 0; j < nama_len && pos_tab <= kolom; j++) {
                    char glyph[8];
                    int glen = panjang_cp_utf8(nama + j);
                    if (j + glen <= nama_len) {
                        memcpy(glyph, nama + j, glen);
                        glyph[glen] = '\0';
                ...
                /* Tab biasa: tulis nama + spasi di kanan */
                int j;
                for (j = 0; j < nama_len && pos_tab < kolom; j++) {
                    char glyph[8];
                    int glen = panjang_cp_utf8(nama + j);
        ...
        if (ada_lebih_kanan && pos_tab <= kolom) {
            warna_set_buffer(WARNAD_KUNING, wl_bg);
            taruh_glyph(pos_tab, y, " ");
            if (pos_tab + 1 <= kolom) {
