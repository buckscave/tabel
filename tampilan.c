            ...
            if (beda) {
                *f = *b;
                if (awal_run < 0) {
                    ...
                    /* Warna berubah, tulis run sebelumnya */
                    atur_posisi_kursor(awal_run + 1, y + 1);

                    if (wd_saat_ini != wd_run || wl_saat_ini != wl_run) {
                        seq_len = warna_seq_gabung(wd_run, wl_run, seq_buf,
                                                   sizeof(seq_buf));
                        if (seq_len > 0) {
                                   ...
                                   struct rect area, int is_active)
{
    ...
    if (buf && buf->lembar && buf->jumlah_lembar > 0) {
        ...
        /* Cek apakah ada tab lebih di kanan */
        if (vis_akhir < jumlah - 1) {
            ada_lebih = 1;
        }

        /* Hitung lebar untuk sheet tabs dari kanan */
        lebar_lembar = 0;
        for (i = vis_awal; i <= vis_akhir; i++) {
            lebar_lembar += (int)strlen(buf->lembar[i]->nama) + 3;
        }
        if (ada_lebih) {
            lebar_lembar += 2;
        }
        ...
        awal_lembar = kolom - lebar_lembar + 1;
        ...
        for (i = vis_awal; i <= vis_akhir && pos_tab < kolom; i++) {
            char tab_buf[NAMA_LEMBAR_MAKS + 8];
            int tab_len;
            int sisa;

            snprintf(tab_buf, sizeof(tab_buf), " %s ",
                     buf->lembar[i]->nama);
            tab_len = (int)strlen(tab_buf);
            sisa = kolom - pos_tab + 1;
            ...
            taruh_teks_elipsis(pos_tab, y, tab_buf, sisa);
            pos_tab += tab_len;
        }

        /* Gambar indikator lebih */
        if (ada_lebih && pos_tab < kolom) {
            warna_set_buffer(WARNAD_KUNING, wl_bg);
            taruh_teks(pos_tab, y, ">");
        } else if (vis_akhir < jumlah - 1 && pos_tab >= kolom) {
            /* Tidak cukup ruang, skip */
        }

        /* Jika di akhir tab dan ada tab sebelumnya, tampilkan "<" */
        if (vis_awal > 0 && awal_lembar > area.x + 1) {
            warna_set_buffer(WARNAD_KUNING, wl_bg);
            taruh_teks(awal_lembar - 2, y, "<");
        }
    }
    ...
    for (i = 0; i < jumlah_buffer && pos_x < ruang_tab; i++) {
        ...
        wl = (i == buffer_aktif) ? WARNAL_HIJAU : wl_bg;
        warna_set_buffer(WARNAD_DEFAULT, wl);
    ...
    struct rect area, int is_active)
{
    ...
    warna_set_buffer(WARNAD_DEFAULT, wl_bg_kanan);
