 ...
 * UTF-8 & wcwidth
 * ============================================================ */
static int adalah_kontinu(unsigned char c) { return (c & 0xC0) == 0x80; }
...
int panjang_cp_utf8(const char *p) {
    ...
    if ((c & 0xF0) == 0xE0) {
        if (!adalah_kontinu((unsigned char)p[1]) || !adalah_kontinu((unsigned char)p[2])) return 1;
        return 3;
    }
    if ((c & 0xF8) == 0xF0) {
        if (!adalah_kontinu((unsigned char)p[1]) || !adalah_kontinu((unsigned char)p[2]) || !adalah_kontinu((unsigned char)p[3])) return 1;
        return 4;
    }
    return 1;
}
...
unsigned int dekode_cp_utf8(const char *p, int *len) {
    ...
    if ((c & 0xE0)==0xC0) cp=((unsigned int)(p[0]&0x1F)<<6)|(unsigned int)(p[1]&0x3F);
    else if ((c & 0xF0)==0xE0) cp=((unsigned int)(p[0]&0x0F)<<12)|((unsigned int)(p[1]&0x3F)<<6)|(unsigned int)(p[2]&0x3F);
    else if ((c & 0xF8)==0xF0) cp=((unsigned int)(p[0]&0x07)<<18)|((unsigned int)(p[1]&0x3F)<<12)|((unsigned int)(p[2]&0x3F)<<6)|(unsigned int)(p[3]&0x3F);
    else cp=c;
    return cp;
}
...
static int lebar_ucs(unsigned int cp) {
    ...
    if ((cp>=0x1100&&cp<=0x115F)||(cp>=0x2E80&&cp<=0xA4CF)||(cp>=0xAC00&&cp<=0xD7A3)||(cp>=0xF900&&cp<=0xFAFF)||(cp>=0xFE10&&cp<=0xFE19)||(cp>=0xFE30&&cp<=0xFE6F)||(cp>=0xFF00&&cp<=0xFF60)||(cp>=0xFFE0&&cp<=0xFFE6)||(cp>=0x1F300&&cp<=0x1FAFF)) return 2;
    return 1;
}

int lebar_tampilan_string_utf8(const char *s) {
    int w=0; const char *p=s; if (!s) return 0;
    while(*p){int l; unsigned int cp=dekode_cp_utf8(p,&l); w+=lebar_ucs(cp); p+=l;}
    return w;
}
int lebar_tampilan_utf8(const char *p){return lebar_tampilan_string_utf8(p);}

/* ============================================================
 * Helper String
 * ============================================================ */
int salin_str_aman(char *dst,const char *src,size_t n){
    size_t i=0; if(!dst||!src||n==0) return 0;
    while(i+1<n&&src[i]!='\0'){dst[i]=src[i];i++;} dst[i]='\0'; return (int)i;
}
int gabung_str_aman(char *dst,const char *src,size_t n){
    size_t dlen,i=0; if(!dst||!src||n==0) return 0; dlen=strlen(dst);
    if(dlen>=n) return (int)dlen;
    while(dlen+i+1<n&&src[i]!='\0'){dst[dlen+i]=src[i];i++;} dst[dlen+i]='\0'; return (int)(dlen+i);
}
...
int fsync_berkas(FILE *f){
    int fd; if(!f) return -1; fd=fileno(f); if(fd<0) return -1;
    if(fflush(f)!=0) return -1; if(fsync(fd)!=0) return -1; return 0;
}
...
void minta_nama_berkas(char *buf, size_t n, const char *label) {
    ...
    int len = 0, cursor = 0;
    int cols = ambil_lebar_terminal();
    int rows = ambil_tinggi_terminal();   /* baris terakhir */
    int pos_x_start = (int)strlen(label) + 2; /* setelah "Buka: " */
    ...
    int ret = baca_input_edit(input_buf, &len, &cursor, pos_x_start, rows);
...
int simpan_txt(const struct buffer_tabel *buf,const char *fn){
    FILE *f; int r,c;
    if(!buf||!fn) return -1;
    f=fopen(fn,"w"); if(!f) return -1;
    for(r=0;r<buf->cfg.baris;r++){
        for(c=0;c<buf->cfg.kolom;c++){
            if(fprintf(f,"%s",buf->isi[r][c])<0){fclose(f);return -1;}
            if(c<buf->cfg.kolom-1) fputc('\t',f);
        }
        fputc('\n',f);
    }
    if(fsync_berkas(f)!=0){fclose(f);return -1;}
    fclose(f); return 0;
}

int buka_txt(struct buffer_tabel **pbuf, const char *fn) {
    FILE *f; char line[MAKS_TEKS*MAKS_KOLOM];
    int baris=0, kolom=0;
...
int simpan_csv(const struct buffer_tabel *buf,const char *fn){
    ...
    for(r=0;r<buf->cfg.baris;r++){
        for(c=0;c<buf->cfg.kolom;c++){
            ...
            int perlu_quote=strchr(val,',')||strchr(val,'"')||strchr(val,'\n');
...
int buka_csv(struct buffer_tabel **pbuf, const char *fn) {
    ...
    while (fgets(line, sizeof(line), f)) {
        baris++;
        int c=0; int last_nonempty=0;
        char *p=line; int inquote=0; int fieldlen=0;
        while (*p) {
    ...
    *pbuf = buat_buffer(baris, kolom);
    ...
    while (fgets(line,sizeof(line),f) && r<(*pbuf)->cfg.baris) {
        ...
        while (*p && c<(*pbuf)->cfg.kolom) {
            char cell[MAKS_TEKS]; int idx=0; int quoted=0;
            if (*p=='"'){quoted=1;p++;}
            while(*p && idx<MAKS_TEKS-1){
                if(quoted){
                    if(*p=='"'){
                        if(*(p+1)=='"'){cell[idx++]='"';p+=2;continue;}
                        else{p++;break;}
                    }
                } else {
                    if(*p==','||*p=='\n') break;
    ...
    *pbuf = buat_buffer(baris, kolom);
    ...
    while(fgets(line,sizeof(line),f)){
        if(strncmp(line,"INSERT",6)==0){
            ...
            int c=0;
            while(*p && c<(*pbuf)->cfg.kolom){
                char cell[MAKS_TEKS]; int idx=0;
                if(*p=='\'') p++;
                while(*p && *p!='\'' && idx<MAKS_TEKS-1){cell[idx++]=*p;p++;}
                cell[idx]='\0';
                salin_str_aman((*pbuf)->isi[r][c],cell,MAKS_TEKS);
                while(*p && *p!=',' && *p!=')') p++;
                if(*p==',') p++;
    ...
    *pbuf = buat_buffer(rows, cols);
    ...
    for(int r=0;r<rows;r++)
        for(int c=0;c<cols;c++){
            size_t len; fread(&len,sizeof(size_t),1,f);
...
int buka_tsv(struct buffer_tabel **pbuf, const char *fn) {
    return buka_txt(pbuf, fn); /* sama persis dengan TXT karena delimiter tab */
}
...
int buka_md(struct buffer_tabel **pbuf, const char *fn) {
    ...
    while (fgets(line, sizeof(line), f)) {
        if (strchr(line,'|')) {
            baris++;
            int c=0; char *tok=strtok(line,"|");
            while(tok){ if(strlen(tok)>0) c++; tok=strtok(NULL,"|"); }
            if (c > kolom) kolom = c;
        }
    }
    fclose(f);
 ...
 * Implementasi ada di xlsx.txt
 * ============================================================ */

/*
 * Catatan: Fungsi buka_xlsx dan simpan_xlsx dideklarasikan di tabel.h
 * dan diimplementasikan di xlsx.txt. File ini hanya berisi wrapper
 * jika diperlukan untuk kompatibilitas.
 */

/* ============================================================
 * I/O XLS (Excel 97-2003 Format)
 * Implementasi ada di xls.txt
 * ============================================================ */

/*
 * Catatan: Fungsi buka_xls dan simpan_xls dideklarasikan di tabel.h
 * dan diimplementasikan di xls.txt.
 */

/* ============================================================
 * I/O XML Spreadsheet (Excel 2002-2003 XML Format)
 * Implementasi ada di xml_spreadsheet.txt
 * ============================================================ */

/*
 * Catatan: Fungsi buka_xml dan simpan_xml dideklarasikan di tabel.h
 * dan diimplementasikan di xml_spreadsheet.txt.
 */

/* ============================================================
 * I/O ODS (OpenDocument Spreadsheet)
 * Implementasi ada di ods.txt
 * ============================================================ */

/*
 * Catatan: Fungsi buka_ods dan simpan_ods dideklarasikan di tabel.h
 * dan diimplementasikan di ods.txt.
 */
...
int deteksi_format_file(const char *fn) {
    ...
    if (strcasecmp(ext, ".htm") == 0) return 13;  /* HTML (alternate) */
...
int buka_file_otomatis(struct buffer_tabel **pbuf, const char *fn) {
        ...
        default:
            /* Coba deteksi berdasarkan isi file */
            {
                ...
                if (fp) {
                    ...
                    size_t bytes = fread(buf, 1, 256, fp);
                    fclose(fp);
                    
                    /* Cek signature TBL (Native)
                     * "TBL3" = 0x334C4254 little-endian */
                    if (bytes >= 4) {
                    ...
                    if (bytes >= 8 && memcmp(buf, "\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1", 8) == 0) {
                        ok = buka_xls(pbuf, fn);
                        break;
                    }
    ...
    if (ok == 0) {
        if (getcwd(cwd, sizeof(cwd))) {
            snprintf((*pbuf)->cfg.path_file, sizeof((*pbuf)->cfg.path_file), 
                     "%s/%s", cwd, fn);
        } else {
            salin_str_aman((*pbuf)->cfg.path_file, fn, sizeof((*pbuf)->cfg.path_file));
        }
    }
