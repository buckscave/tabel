...
extern void taruh_teks(int x, int y, const char *s, warna_glyph w);
...
void paksa_recalc_layout(void) {
    ...
    /* * Jendela mulai dari y=1 (menyisakan y=0 untuk Tab Global).
     * Tinggi dikurangi 2 (1 untuk Tab atas, 1 untuk Pesan bawah).
     */
    update_layout_rekursif(jendela_root, 0, 1, w, h - 2);
}
...
void render_tree_jendela(struct jendela *node) {
    ...
    if (node->jenis == JENIS_DAUN) {
        ...
        /* Backup state buffer */
        int old_ax = b->cfg.aktif_x, old_ay = b->cfg.aktif_y;
        int old_lx = b->cfg.lihat_kolom, old_ly = b->cfg.lihat_baris;
        ...
        if (node->jenis == JENIS_SPLIT_V) {
            ...
            for (int i = node->rect.y; i < node->rect.y + node->rect.h; i++) {
                taruh_teks(split_x, i + 1, V, WARNA_ABU);
            }
        }
    }
}
