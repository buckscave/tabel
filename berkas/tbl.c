...
/* ============================================================
 * PERBAIKAN: Border data dengan warna depan dan latar
 * ============================================================ */
struct tbl_border {
    int row;
    int col;
...
/* ============================================================
 * PERBAIKAN: Color data dengan kombinasi depan dan latar
 * ============================================================ */
struct tbl_color {
    int row;
    int col;
...
/* ============================================================
 * PERBAIKAN: Write border section dengan warna depan dan latar
 * ============================================================ */
    ...
    for (r = 0; r < buf->cfg.baris; r++) {
        for (c = 0; c < buf->cfg.kolom; c++) {
            ...
            if (b->top || b->bottom || b->left || b->right) {
                ...
                /* PERBAIKAN: Simpan warna depan dan latar */
...
/* ============================================================
 * PERBAIKAN: Write color section dengan warna depan dan latar
 * ============================================================ */
static int tbl_write_color(FILE *fp, const struct buffer_tabel *buf) {
    ...
    if (!buf->warna_sel) return 0;
    
    /* Count cells with non-default color */
    for (r = 0; r < buf->cfg.baris; r++) {
        for (c = 0; c < buf->cfg.kolom; c++) {
            struct warna_kombinasi *wc = &buf->warna_sel[r][c];
            /* Simpan jika warna depan atau latar tidak default */
...
int simpan_tbl(const struct buffer_tabel *buf, const char *fn) {
    ...
    if (buf->warna_sel) flags |= TBL_FLAG_HAS_COLOR;
...
/* ============================================================
 * PERBAIKAN: Read border section dengan warna depan dan latar
 * ============================================================ */
static int tbl_read_border(FILE *fp, struct buffer_tabel *buf, unsigned short count) {
    ...
    for (i = 0; i < count; i++) {
        ...
        unsigned char warna_depan, warna_latar;
        ...
        /* PERBAIKAN: Baca warna depan dan latar */
        if (read_byte(fp, &warna_depan) != 0) return -1;
        if (read_byte(fp, &warna_latar) != 0) return -1;
        ...
        if (row >= 0 && row < buf->cfg.baris && 
            ...
            if (buf->border_sel) {
                ...
                /* PERBAIKAN: Set warna depan dan latar */
                buf->border_sel[row][col].warna_depan = (warna_depan);warna_depan;
                buf->border_sel[row][col].warna_latar = (warna_latar);warna_latar;
            }
        }
    }
...
/* ============================================================
 * PERBAIKAN: Read color section dengan warna depan dan latar
 * ============================================================ */
static int tbl_read_color(FILE *fp, struct buffer_tabel *buf, unsigned short count) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (row >= 0 && row < buf->cfg.baris && 
            col >= 0 && col < buf->cfg.kolom) {
            if (buf->warna_sel) {
                buf->warna_sel[row][col].depan = (warna_depan);warna_depan;
                buf->warna_sel[row][col].latar = (warna_latar);warna_latar;
            }
        }
    }
    ...
    while (tbl_read_section(fp, &sec) == 0) {
        ...
        switch (sec.type) {
            ...
            case TBL_SECTION_COLOR:
                /* PERBAIKAN: Ensure color array exists */
                if (!buf->warna_sel && buf->cfg.baris > 0 && buf->cfg.kolom > 0) {
                    int r;
                    buf->warna_sel = calloc(buf->cfg.baris, sizeof(struct warna_kombinasi *));
                    if (buf->warna_sel) {
                        for (r = 0; r < buf->cfg.baris; r++) {
                            buf->warna_sel[r] = calloc(buf->cfg.kolom, sizeof(struct warna_kombinasi));
                        }
                    }
                }
