...
struct info_border {
    ...
    warna_glyph warna;              /* warna border */
};

/* ============================================================
 * Struktur Buffer (Multi Buffer)
 * ============================================================ */
struct buffer_tabel {
...
void input_reg_cb(callback_t kc, 
        ...
        callback_t pc, 
        void *ctx);
static int input_poll_int(InputEvent *e, int blk, int to);
...
void render_jendela_spesifik(const struct buffer_tabel *buf, struct rect area, int is_active);
void set_clip_rect(int x, int y, int w, int h);
void reset_clip_rect(void);
void terapkan_delta(void); /* Expose this if needed by jendela.c */
